...
# ── Validación de Hardware (Edge Server) ───────────────────────
info "Verificando especificaciones para el rol de Edge Server..."
TOTAL_RAM=$(free -g | awk '/^Mem:/{print $2}')
CPU_CORES=$(nproc)
if [[ $TOTAL_RAM -lt 30 ]]; then
    echo -e "${AMARILLO}[!] ADVERTENCIA: Se recomiendan 32GB de RAM para el rol de Edge Server.${NC}"
    echo -e "    RAM detectada: ${TOTAL_RAM}GB"
fi
if [[ $CPU_CORES -lt 8 ]]; then
    echo -e "${AMARILLO}[!] ADVERTENCIA: Se recomienda un procesador de 8+ núcleos (i7/Ryzen 7).${NC}"
    echo -e "    Núcleos detectados: ${CPU_CORES}"
fi
...
# Dominios locales fáciles para el personal
address=/salud.local/${LAN_IP}
address=/bioko.local/${LAN_IP}
address=/historia.local/${LAN_IP}
address=/salud/${LAN_IP}
# Cache DNS para mayor velocidad
cache-size=1000
DNSMASQ
...
# Optimización para 32GB RAM
if [[ $TOTAL_RAM -ge 30 ]]; then
    info "Aplicando optimización de base de datos para 32GB RAM..."
    cat >> /etc/mysql/conf.d/bioko.cnf << MYSQL_OPT
[mysqld]
